...
const ProductDisplay = (props) => {
  ...
  useEffect(() => {
    const checkWishlist = async () => {
      ...
      try {
        const response = await fetch(`${API_URL}/wishlist/check/${product.id}`, {
          headers: {
        ...
        if (data.success) {
          setIsInWishlist(data.data.isInWishlist || false);
        }
      } catch (err) {
    ...
    <div className='productdisplay'>
      ...
      <div className="productdisplay-right">
        ...
        <div className="action-buttons">
          ...
          >
            <svg viewBox="0 0 24 24" fill={isInWishlist ? 'currentColor' : 'none'}>
